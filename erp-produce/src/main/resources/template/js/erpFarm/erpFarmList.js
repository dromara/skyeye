...
layui.config({
    ...
    function deletemember(data){
        layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
            AjaxPostUtil.request({url:reqBasePath + "erpfarm005", params: {rowId: data.id}, type: 'json', callback: function(json){
    ...
    function norms(data){
        layer.confirm('确认要恢复正常吗？', { icon: 3, title: '恢复操作' }, function (index) {
            AjaxPostUtil.request({url:reqBasePath + "erpfarm008", params: {rowId: data.id}, type: 'json', callback: function(json){
                if(json.returnCode == 0){
    ...
    function rectification(data){
        layer.confirm('确认要维修整改吗？', { icon: 3, title: '维修整改操作' }, function (index) {
            AjaxPostUtil.request({url:reqBasePath + "erpfarm009", params: {rowId: data.id}, type: 'json', callback: function(json){
                if(json.returnCode == 0){
                    winui.window.msg("提交成功。", {icon: 1,time: 2000});
                    loadTable();
                }else{
