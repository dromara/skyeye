...
layui.config({
    ...
    table.render({
        ...
        elem: '#messageTable',
        method: 'post',
        url: sysMainMation.erpBasePath + 'erppick003',
    ...
    table.on('tool(messageTable)', function (obj) {
        ...
        }else if (layEvent === 'turnPatchMaterialToTurnOut') { //转补料出库
            turnPatchMaterialToTurnOut(data);
    ...
    $("body").on("click", "#addBean", function() {
        _openNewWindows({
            url:  systemCommonUtil.getUrl('FP2023100600001', null),
            title: systemLanguage["com.skyeye.addPageTitle"][languageType],
            pageId: "erpPatchPickAdd",
            area: ['90vw', '90vh'],
            callBack: function (refreshCode) {
                winui.window.msg(systemLanguage["com.skyeye.successfulOperation"][languageType], {icon: 1, time: 2000});
                loadTable();
    ...
    function edit(data) {
        _openNewWindows({
            url:  systemCommonUtil.getUrl('FP2023100600002&id=' + data.id, null),
            title: systemLanguage["com.skyeye.editPageTitle"][languageType],
            pageId: "erpPatchPickEdit",
            area: ['90vw', '90vh'],
            callBack: function (refreshCode) {
                winui.window.msg(systemLanguage["com.skyeye.successfulOperation"][languageType], {icon: 1, time: 2000});
                loadTable();
    ...
    function details(data) {
        _openNewWindows({
            url:  systemCommonUtil.getUrl('FP2023100600003&id=' + data.id, null),
            title: systemLanguage["com.skyeye.detailsPageTitle"][languageType],
            pageId: "erpPatchPickDetails",
            area: ['90vw', '90vh'],
            callBack: function (refreshCode) {
            }});
    }
    ...
    function del(data) {
        layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function (index) {
            AjaxPostUtil.request({url: sysMainMation.erpBasePath + "deletePatchMaterialById", params: {id: data.id}, type: 'json', method: 'DELETE', callback: function (json) {
	...
	function subApproval(data) {
        layer.confirm('确认要提交审核吗？', { icon: 3, title: '提交审核操作' }, function (index) {
            activitiUtil.startProcess(data.serviceClassName, null, function (approvalId) {
                ...
                AjaxPostUtil.request({url: sysMainMation.erpBasePath + "submitPatchMaterial", params: params, type: 'json', method: 'POST', callback: function (json) {
    ...
    function revoke(data) {
        layer.confirm('确认撤销该申请吗？', { icon: 3, title: '撤销操作' }, function (index) {
            layer.close(index);
            AjaxPostUtil.request({url: sysMainMation.erpBasePath + "revokePatchMaterial", params: {processInstanceId: data.processInstanceId}, type: 'json', method: "PUT", callback: function (json) {
                winui.window.msg("提交成功", {icon: 1, time: 2000});
                loadTable();
            }});
        });
