...
layui.config({
    ...
    table.render({
        ...
        elem: '#messageTable',
        method: 'post',
        url: reqBasePath + 'wagesmodeltype001',
    ...
    function delet(data){
        layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
            layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "wagesmodeltype005", params:{rowId: data.id}, type: 'json', method: "DELETE", callback: function(json){
    ...
    function down(data){
        layer.confirm(systemLanguage["com.skyeye.disableOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.disableOperation"][languageType]}, function(index) {
            layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "wagesmodeltype007", params:{rowId: data.id}, type: 'json', method: "GET", callback: function(json){
                if (json.returnCode == 0) {
    ...
    function up(data){
        layer.confirm(systemLanguage["com.skyeye.enableOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.enableOperation"][languageType]}, function(index) {
            layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "wagesmodeltype006", params:{rowId: data.id}, type: 'json', method: "GET", callback: function(json){
                if (json.returnCode == 0) {
