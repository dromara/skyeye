...
layui.config({
	...
	authBtn('1582381689724');//新增

	table.render({
		id: 'messageTable',
		elem: '#messageTable',
		method: 'post',
		url: sysMainMation.sealServiceBasePath + 'sealseservice023',
	...
	$("body").on("click", "#addBean", function() {
		_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023091000001', null),
	...
	function edit(data) {
		_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023091000002&id=' + data.id, null),
	...
	function details(data) {
		_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023091000003&id=' + data.id, null),
	...
	function del(data) {
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: sysMainMation.sealServiceBasePath + "deleteSealApplyById", params: {id: data.id}, type: 'json', method: "DELETE", callback: function (json) {
	...
	function subApproval(data) {
		layer.confirm(systemLanguage["com.skyeye.approvalOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.approvalOperation"][languageType]}, function (index) {
			...
			activitiUtil.startProcess(data.serviceClassName, null, function (approvalId) {
				...
				AjaxPostUtil.request({url: sysMainMation.sealServiceBasePath + "submitSealApplyToApproval", params: params, type: 'json', method: "POST", callback: function (json) {
					winui.window.msg("提交成功", {icon: 1, time: 2000});
					loadTable();
				}});
			});
