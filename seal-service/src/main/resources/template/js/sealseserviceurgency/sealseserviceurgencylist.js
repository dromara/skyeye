...
layui.config({
	...
	authBtn('1576205926050');
	table.render({
	    id: 'messageTable',
	    elem: '#messageTable',
	    method: 'post',
	    url: flowableBasePath + 'sealseserviceurgency001',
	...
	$("body").on("click", "#addBean", function(){
    	_openNewWindows({
			url: "../../tpl/sealseserviceurgency/sealseserviceurgencyadd.html", 
			title: systemLanguage["com.skyeye.addPageTitle"][languageType],
			pageId: "sealseserviceurgencyadd",
			area: ['40vw', '40vh'],
	...
	function edit(data){
		rowId = data.id;
		_openNewWindows({
			url: "../../tpl/sealseserviceurgency/sealseserviceurgencyedit.html", 
			title: systemLanguage["com.skyeye.editPageTitle"][languageType],
			pageId: "sealseserviceurgencyedit",
			area: ['40vw', '40vh'],
			callBack: function(refreshCode){
                if (refreshCode == '0') {
                	winui.window.msg(systemLanguage["com.skyeye.successfulOperation"][languageType], {icon: 1,time: 2000});
	...
	function del(data, obj){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "sealseserviceurgency005", params: {rowId: data.id}, type: 'json', callback: function(json){
	...
	function up(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '上线操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "sealseserviceurgency006", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function down(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '下线操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "sealseserviceurgency007", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
    ...
    exports('sealseserviceurgencylist', {});
});