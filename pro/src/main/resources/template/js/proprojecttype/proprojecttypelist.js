...
layui.config({
	...
	authBtn('1574066705504');
	table.render({
	    id: 'messageTable',
	    elem: '#messageTable',
	    method: 'post',
	    url: flowableBasePath + 'proprojecttype001',
	...
	$("body").on("click", "#addBean", function() {
    	_openNewWindows({
			url: "../../tpl/proprojecttype/proprojecttypeadd.html", 
			title: systemLanguage["com.skyeye.addPageTitle"][languageType],
			pageId: "proprojecttypeadd",
			area: ['90vw', '90vh'],
	...
	function edit(data){
		rowId = data.id;
		_openNewWindows({
			url: "../../tpl/proprojecttype/proprojecttypeedit.html", 
			title: systemLanguage["com.skyeye.editPageTitle"][languageType],
			pageId: "proprojecttypeedit",
			area: ['90vw', '90vh'],
			callBack: function(refreshCode){
				winui.window.msg(systemLanguage["com.skyeye.successfulOperation"][languageType], {icon: 1, time: 2000});
				loadTable();
	...
	function del(data, obj){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "proprojecttype005", params: {rowId: data.id}, type: 'json', callback: function (json) {
	...
	function up(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '上线操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "proprojecttype006", params: {rowId: data.id}, type: 'json', callback: function (json) {
    			if (json.returnCode == 0) {
	...
	function down(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '下线操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: flowableBasePath + "proprojecttype007", params: {rowId: data.id}, type: 'json', callback: function (json) {
    			if (json.returnCode == 0) {
