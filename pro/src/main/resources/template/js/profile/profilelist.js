...
layui.config({
	...
	function revoke(data){
		...
		layer.confirm(msg, { icon: 3, title: '撤销申请' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "profile007", params:{processInstanceId: data.processInstanceId}, type:'json', callback:function(json){
	...
	function subApproval(data, obj){
		layer.confirm(systemLanguage["com.skyeye.approvalOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.approvalOperation"][languageType]}, function (index) {
			...
			activitiUtil.startProcess(sysActivitiModel["proFile"]["key"], function (approvalId) {
				...
				AjaxPostUtil.request({url: reqBasePath + "profile006", params: params, type: 'json', callback: function(json){
					if(json.returnCode == 0){
						winui.window.msg("提交成功", {icon: 1, time: 2000});
						loadTable();
					}else{
