...
layui.config({
	...
	function del(data){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "proproject009", params: {rowId: data.id}, type: 'json', callback: function(json){
	...
	function revoke(data){
		layer.confirm("确定撤销申请吗？", {icon: 3, title: '撤销操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "proproject010", params: {processInstanceId: data.processInstanceId}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function nullify(data){
		layer.confirm("确定作废该项目吗？", {icon: 3, title: '作废操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "proproject011", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function execute(data){
		layer.confirm("确定开始执行该项目吗？", {icon: 3, title: '执行操作'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "proproject012", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function subApproval(data){
		layer.confirm(systemLanguage["com.skyeye.approvalOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.approvalOperation"][languageType]}, function (index) {
			...
			activitiUtil.startProcess(sysActivitiModel["proProject"]["key"], function (approvalId) {
				...
				AjaxPostUtil.request({url: reqBasePath + "proproject008", params: params, type: 'json', callback: function(json){
					if(json.returnCode == 0){
