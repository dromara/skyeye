...
layui.config({
	...
	table.render({
	    ...
	    elem: '#messageTable',
	    method: 'post',
	    url: reqBasePath + 'procostexpensetype001',
	...
	function del(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '删除操作'}, function (index) {
            ...
            AjaxPostUtil.request({url: reqBasePath + "procostexpensetype005", params: {rowId: data.id}, type: 'json', callback: function(json){
	...
	function up(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '上线操作'}, function (index) {
			layer.close(index);
            //向服务端发送上线指令
            AjaxPostUtil.request({url: reqBasePath + "procostexpensetype006", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function down(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '下线操作'}, function (index) {
			layer.close(index);
            //向服务端发送下线指令
            AjaxPostUtil.request({url: reqBasePath + "procostexpensetype007", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
