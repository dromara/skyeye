...
layui.config({
	...
	// 加载动态表单
	dsFormUtil.loadPageByCode("dsFormShow", sysDsFormWithCodeType["putIsOthers"]["code"]);
	...
	form.on('submit(formSubOneBean)', function(data) {
		if(winui.verifyForm(data.elem)) {
			activitiUtil.startProcess(sysActivitiModel["putIsOthers"]["key"], function (approvalId) {
	...
	function saveData(subType, approvalId){
		...
		AjaxPostUtil.request({url: reqBasePath + "otherwarehous002", params: params, type: 'json', method: "POST", callback: function (json) {
		...
		_openNewWindows({
			...
			callBack: function (refreshCode) {
				if (refreshCode == '0') {
					...
					winui.window.msg(systemLanguage["com.skyeye.operationFailed"][languageType], {
						icon: 2,
						time: 2000
					});
				}
