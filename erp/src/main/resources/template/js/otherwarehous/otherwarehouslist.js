...
layui.config({
    ...
    table.render({
        ...
        elem: '#messageTable',
        method: 'post',
        url: flowableBasePath + 'otherwarehous001',
    ...
    table.on('tool(messageTable)', function (obj) {
        ...
        if (layEvent === 'delete') { //删除
            ...
            erpOrderUtil.revokeOrderMation(data.processInstanceId, systemOrderType["putIsOthers"]["orderType"], function() {
                loadTable();
            });
        }
    });
	...
	function subExamine(data){
        erpOrderUtil.submitOrderMation(data.id, systemOrderType["putIsOthers"]["orderType"], data.submitType, sysActivitiModel["putIsOthers"]["key"], function() {
            loadTable();
        });
    }
    ...
    function deletemember(data){
        erpOrderUtil.deleteOrderMation(data.id, systemOrderType["putIsOthers"]["orderType"], function() {
            loadTable();
        });
    }
