...
layui.config({
	...
	function initDataShow(){
		AjaxPostUtil.request({url: flowableBasePath + "salesorder004", params: {rowId: parent.rowId}, type: 'json', method: "GET", callback: function(json) {
			if(json.returnCode == 0) {
				...
				// 是否需要统筹
				$("input:radio[name=needOverPlan][value=" + json.bean.needOverPlan + "]").attr("checked", true);
	...
	form.on('submit(formSubOneBean)', function(data) {
		if(winui.verifyForm(data.elem)) {
			activitiUtil.startProcess(sysActivitiModel["outchaseOrder"]["key"], function (approvalId) {
	...
	function saveData(subType, approvalId){
		...
		var params = {
			...
			salesMan: salesMan,
			needOverPlan: $("input[name='needOverPlan']:checked").val(),
			subType: subType,
			submitType: submitType,
			approvalId: approvalId,
			rowId: parent.rowId
		};
		AjaxPostUtil.request({url: flowableBasePath + "salesorder005", params: params, type: 'json', method: "PUT", callback: function(json) {
			if(json.returnCode == 0) {
