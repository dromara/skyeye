	...
	table.render({
		...
		cols: [[
			...
			{ field: 'fromTypeId', title: '来源类型', width: 150, templet: function (d) {
				return skyeyeClassEnumUtil.getEnumDataNameByCodeAndKey("sealOrderFromType", 'id', d.fromTypeId, 'name');
			}},
			...
			for (var i = 0; i < json.rows.length; i++) {
				// 只有【审核通过、部分完成】的销售订单才可以进行选中
				let state = json.rows[i].state;
				if (state == 'pass' || state == 'partiallyCompleted') {
