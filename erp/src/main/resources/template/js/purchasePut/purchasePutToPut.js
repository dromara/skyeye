...
layui.config({
    ...
    // 采购入库转仓库入库单
    AjaxPostUtil.request({url: sysMainMation.erpBasePath + "queryPurchasePutTransById", params: {id: id}, type: 'json', method: 'GET', callback: function (json) {
        let data = json.bean;
        // 仓库入库的【编辑布局】
        dsFormUtil.initEditPageForStatic('content', 'FP2024070100009', data, {
            ...
            saveData: function (params) {
                // 保存数据
                AjaxPostUtil.request({url: sysMainMation.erpBasePath + "insertPurchasePutToTurnDepot", params: params, type: 'json', method: "POST", callback: function(json) {
