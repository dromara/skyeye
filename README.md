...
## 📖 项目简介
SkyEye云 是面向**企业级智能制造行业**的`AI+`一站式`SaaS化`零代码全链路管理平台，一套底座覆盖企业数字化全场景：
内置CRM客户管理、ERP进销存、MES生产制造、WMS仓储、OA行政办公、EHR人事薪资、项目管理、商城、财务、售后工单、工作流引擎、智能报表等**上百种业务模块**。
依托自研零代码设计器，无需编写原生CRUD代码，拖拽式快速搭建业务表单、流程、打印模板，支持PC端+移动端（小程序/H5/App）多端同源发布。
...
## 📚  源码与商用版本
> 开源仓库不包含所有代码，**全量业务源码需开通Skyeye会员获取**，会员源码支持：学习研究、毕业设计、企业商用二开。本项目支持本地私有部署。所有功能无使用限制。
...
| 场景 | 能力 |
|----|----|
|  AI 知识库  |  自动同步私有库的数据进行切片分段，可选择性的进行数据同步，文件关联与定时同步，支撑项目内检索问答  |
|  AI 对话  |  自由对话，结合 AI 知识库回答项目内知识，减少乱答  |
|  AI 布局设计器  |  AI 是给低代码加上**智能生成、自动优化、自然语言交互**能力。低代码是减少代码量，AI 是降低 “搭建和配置” 的脑力成本；AI 低代码把 “会拖拽配置” 的门槛，进一步降到 “会说清楚需求”。  |
| AI智能填单 | 内置 AI 智能填单功能，支持上传单据、图片、文本自动提取信息并填充表单，告别手工重复录入，减少数据错误，业务人员上传材料即可快速生成业务单据。 |
...
|----|----|
|  AI 对话  |  自由对话，结合 AI 知识库回答项目内知识，减少乱答  |
|  需求  |  一句话生成需求文档结构（背景、范围、前后端/测试任务、验收标准等）  |
|  冒烟用例  |  按需求 + 模块 API 目录生成用例步骤  |
| Bug   |  文字 + 截图识图生成缺陷单  |
|  步骤断言  | 根据试跑输出 AI 生成断言并写回步骤   |
|步骤入参| 根据接口定义 / 上下文 AI 解析并生成步骤入参|
|执行分析|用例执行失败后 AI 分析原因，支持一键生成 Bug 草稿|
