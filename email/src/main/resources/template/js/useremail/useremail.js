...
layui.config({
	...
	AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail001", params: {}, type: 'json', callback: function (json) {
    ...
    form.on('submit(formAddBean)', function (data) {
        if (winui.verifyForm(data.elem)) {
        	AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail002", params: {emailAddress: $("#emailAddress").val(), emailPassword: $("#emailPassword").val()}, type: 'json', callback: function (json) {
	...
	function inboxEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail004", params: {page: page, limit: 12, emailId: $("#checkEmail").attr('rowid')}, type: 'json', callback: function (json) {
	...
	function draftsEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				var lis = [];
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail011", params: {page: page, limit: 12, emailId: $("#checkEmail").attr('rowid')}, type: 'json', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
	...
	function sendedEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				var lis = [];
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail007", params: {page: page, limit: 12, emailId: $("#checkEmail").attr('rowid')}, type: 'json', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
	...
	function deleteedEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				var lis = [];
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail009", params: {page: page, limit: 12, emailId: $("#checkEmail").attr('rowid')}, type: 'json', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
