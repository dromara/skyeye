...
layui.config({
	...
	AjaxPostUtil.request({url: sysMainMation.emailBasePath + "queryEmailListByUserId", params: {}, type: 'json', method: 'GET', callback: function (json) {
    ...
    form.on('submit(formAddBean)', function (data) {
        if (winui.verifyForm(data.elem)) {
        	...
        	AjaxPostUtil.request({url: sysMainMation.emailBasePath + "createEmailUser", params: params, type: 'json', method: 'POST', callback: function (json) {
		...
		flow.load({
			...
			isAuto: true,
				...
				let params = {
					page: page,
					limit: 12,
					objectId: $("#checkEmail").attr('rowid')
				}
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail004", params: params, type: 'json', method: 'POST', callback: function (json) {
	...
	function draftsEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail011", params: params, type: 'json', method: 'POST', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
	...
	function sendedEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail007", params: params, type: 'json', method: 'POST', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
	...
	function deleteedEmailAselFun(){
		...
		flow.load({
			...
			isAuto: true,
				...
				//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
				AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail009", params: params, type: 'json', method: 'POST', callback: function (json) {
					var jsonStr = "";//实体json对象
					$.each(json.rows, function(index, item) {
						jsonStr = {
							bean: item
	...
	$("body").on("click", "#collectEmail", function (e) {
		var _selId = $("#emailSetting").find("a[class='selected']").attr("id");
		if(_selId === 'inboxEmailAsel'){//收件箱
			AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail003", params: {emailUserId: $("#checkEmail").attr('rowid')}, type: 'json', method: 'GET', callback: function (json) {
				winui.window.msg("获取邮件中", {icon: 1, time: 2000});
			}});
		} else if (_selId === 'draftsEmailAsel'){//草稿箱
			AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail010", params: {emailUserId: $("#checkEmail").attr('rowid')}, type: 'json', method: 'GET', callback: function (json) {
				winui.window.msg("获取邮件中", {icon: 1, time: 2000});
			}});
		} else if (_selId === 'sendedEmailAsel'){//已发送邮件
			AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail006", params: {emailUserId: $("#checkEmail").attr('rowid')}, type: 'json', method: 'GET', callback: function (json) {
				winui.window.msg("获取邮件中", {icon: 1, time: 2000});
			}});
		} else if (_selId === 'deleteedEmailAsel'){//已删除邮件
			AjaxPostUtil.request({url: sysMainMation.emailBasePath + "useremail008", params: {emailUserId: $("#checkEmail").attr('rowid')}, type: 'json', method: 'GET', callback: function (json) {
				winui.window.msg("获取邮件中", {icon: 1, time: 2000});
			}});
		} else {
			winui.window.msg('参数错误', {icon: 2, time: 2000});
