...
SELECT
  ...
  '用自然语言描述大屏需求，生成 1920x1080 深色全屏大屏JSON。已配置大模型 Key 时由模型排版，否则回退固定模板。',
  1,
...
CREATE TABLE IF NOT EXISTS `skyeye_ai_skill_exec` (
  ...
  `report_page_id` varchar(32) DEFAULT NULL COMMENT '关联 report_page.id',
  `report_content` mediumtext COMMENT '写入报表页的 content JSON',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='报表页面（Skills本地镜像）';
