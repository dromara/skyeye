...
layui.config({
	...
	table.render({
		...
		elem: '#messageTable',
		method: 'post',
		url: sysMainMation.admBasePath + 'licencerevert001',
	...
	function revoke(data) {
		...
		layer.confirm(msg, { icon: 3, title: '撤销操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: sysMainMation.admBasePath + "licencerevert010", params: {processInstanceId: data.processInstanceId}, type: 'json', method: "PUT", callback: function (json) {
	...
	function edit(data) {
		_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023062500006&id=' + data.id, null),
			title: "编辑证照归还申请",
	...
	function subApproval(data) {
		layer.confirm(systemLanguage["com.skyeye.approvalOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.approvalOperation"][languageType]}, function (index) {
			...
			activitiUtil.startProcess(serviceClassName, null, function (approvalId) {
				...
				AjaxPostUtil.request({url: sysMainMation.admBasePath + "licencerevert006", params: params, type: 'json', method: "POST", callback: function (json) {
					winui.window.msg("提交成功", {icon: 1, time: 2000});
					loadTable();
				}});
			});
	...
	function cancellation(data) {
		...
		layer.confirm(msg, { icon: 3, title: '作废操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: sysMainMation.admBasePath + "licencerevert007", params: {id: data.id}, type: 'json', method: "POST", callback: function (json) {
				winui.window.msg(systemLanguage["com.skyeye.successfulOperation"][languageType], {icon: 1, time: 2000});
				loadTable();
    		}});
		});
	...
	function details(data) {
		_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023062500007&id=' + data.id, null),
	...
	$("body").on("click", "#addBean", function() {
    	_openNewWindows({
			url: systemCommonUtil.getUrl('FP2023062500005', null),
			title: "证照归还申请",
