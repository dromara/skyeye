...
layui.config({
	...
	function assetfromdelet(data){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "assetfrom003", params:{rowId: data.id}, type:'json', callback:function(json){
	...
	function assetfromup(data, obj){
		...
		layer.confirm(msg, { icon: 3, title: '上线操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "assetfrom007", params:{rowId: data.id}, type:'json', callback:function(json){
    			if(json.returnCode == 0){
	...
	function assetfromdown(data, obj){
		...
		layer.confirm(msg, { icon: 3, title: '下线操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url:reqBasePath + "assetfrom008", params:{rowId: data.id}, type:'json', callback:function(json){
    			if(json.returnCode == 0){
	...
	function assetfromtopOne(data){
		AjaxPostUtil.request({url:reqBasePath + "assetfrom009", params:{rowId: data.id}, type:'json', callback:function(json){
			if(json.returnCode == 0){
	...
	function assetfromlowerOne(data){
		AjaxPostUtil.request({url:reqBasePath + "assetfrom010", params:{rowId: data.id}, type:'json', callback:function(json){
			if(json.returnCode == 0){
