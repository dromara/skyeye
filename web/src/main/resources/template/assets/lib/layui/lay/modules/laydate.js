    ...
    */
    ...
    Class.prototype.calendar = function (value) {
        ...
        lay.each(tds, function (index, item) {
            ...
            YMD[2] = st + 1;
            var dayStr = YMD.join('-');
            item.attr('lay-ymd', dayStr).html(YMD[2]);
            ...
            // 加载指定周几可以选择
            if(options.chooseDay != null && options.chooseDay.length > 0){
                // chooseDay=[{"type":1,"day":1},{"type":1,"day":2},{"type":1,"day":3},{"type":1,"day":4},{"type":1,"day":5},{"type":1,"day":6}]
                // type=1:每周都上班  type=2:单周上班
                // 获取周几
                var weekDay = that.weekDay(dayStr);
                var inChooseDay = that.getInPoingArr(options.chooseDay, 'day', weekDay, null);
                if(inChooseDay == null){
                    // 不在指定的周几范围内，设置为不可选择
     ...
     * @returns {number}
     */
    Class.prototype.weekDay = function (date){
     ...
     * @returns {null|*}
     */
    Class.prototype.getInPoingArr = function (array, key, value, getKey){
     ...
     * @returns {number}
     */
    Class.prototype.weekofyear = function (sdate){
        var that = this;
    ...
    Class.prototype.isLeapYear = function (date) {
    ...
    Class.prototype.list = function (type, index) {
        var that = this
            ...
            'class': ELEM_LIST + ' ' + ({
                ...
                , timeminuteCompare: 'laydate-timeminute-list'
            })[type]
        })
        ...
        if (type === 'year') { //年列表
            ...
            setTimeStatus();
        } else if (type === 'timeminuteCompare') { //时间列表,需要和当前时间比较的
            var getMinutes = function(minutes){
                if(options.min.year != 1900){
                    return (minutes - 1);
                }
                if(options.max.year != 1900){
                    minutes = minutes + 1;
                }
                return minutes;
            };
                ...
                lay(ul).find('ol').each(function (i, ol) {
                    lay(ol).find('li').each(function (ii, li) {
                        that.limit(lay(li), [{
                            ...
                            hours: that[startEnd].hours,
                            minutes: getMinutes(ii * options.minutesinterval)
            ...
            lay.each([24, 60], function (i, item) {
                var li = lay.elem('li'), childUL = ['<p>' + lang.time[i] + '</p><ol>'];
                if (item == '60' && !isNull(options.minutesinterval)) {//分钟
