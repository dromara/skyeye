...
layui.define(["jquery"], function(exports) {
    ...
    a.fn.upload = function(D) {
        ...
        var r = a(this),
            ...
            d = a('<input type="file" name="file" class="skyeye-upload"/>'),
        ...
        r.append(A.attr("data-num", B.num).attr("data-type", B.type))
        	.append(d.attr("multiple", B.num > 1 ? "multiple" : false))
        	.append(b.attr("name", B.inputname)).addClass(B.num > 1 ? "multiple" : "one").on("click", "div.add", function(e) {
            d.click();
            // 防止事件冒泡到DOM树上，也就是不触发的任何前辈元素上的事件处理函数。
            e.stopPropagation();
        }).on("change", "input[type='file']", function(H) {//文件
        ...
        // 上传备注
        r.after('<div class="layui-form-mid layui-word-aux">只支持 ' + r.attr("data-type") + ' 格式,最多上传 ' + r.attr("data-num") + ' 个文件</div>');
        
        if (B.value) {
            var picList = B.value.split(","),
                s = 0,
                h = "";
            var f = 0;
            var completeUrl = ""
            r.find("input[type='hidden'][name='upload']").attr("oldurl", B.value);
            for (var x in picList) {
                if (s < B.num && picList[s]) {
                    var ty = picList[s].toLowerCase().split(".").splice(-1).join();
                    if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(ty) >= 0) {
                    	if(options.uploadType == '1'){
                    		h = $("<li class='item success'></li>").append(g).append(typeof D == "function" ? E : "")
                    			.data("url", fileBasePath + "/images/upload/smpropic/" + picList[s]).attr("data-filename", c(picList[s])).css("background-image", "url('" + fileBasePath + "/images/upload/smpropic/" + picList[s] + "')").insertBefore(A);
                    	} else {
                    		h = $("<li class='item success'></li>").append(g).append(typeof D == "function" ? E : "")
                    			.data("url", picList[s]).attr("data-filename", c(picList[s])).css("background-image", "url('" + fileBasePath + picList[s] + "')").insertBefore(A);
                    	}
                    } else {
                        h = $("<li class=\"item success\" data-filename=\"YPCn1FUBacGJOuYKb6\">" + k(ty)+ '<div class="filename">' + ty + '</div>' + "</li>")
                        	.data("url", picList[s]).insertBefore(A);
                    }
                    h.attr("oldurl", picList[s]);
                    r.data("num", ++f).removeClass("empty");
                    q();
                    if(options.uploadType == '1'){
                    	completeUrl += fileBasePath + "\\images\\upload\\smpropic\\" + picList[s] + ",";
                    } else {
                    	completeUrl += fileBasePath + picList[s] + ",";
                    }
                }
                s++;
            }
        ...
        a(document).on("mousemove", function(G) {
            ...
            if (j) {
                if (j.hasClass("delete") && j.data("filename")) {
                    ...
                    r.children("div.add").css("margin-left", 0);
                    return false
                }
                var i = j.clone();
            ...
            H.preventDefault();
            r.removeClass("dragenter");
            if(!H.originalEvent.dataTransfer){
            	return false;
            }
        ...
        function l(G, L) {//检验文件并在canvas中显示
            d.val("");
            if (B.num == 1) {
