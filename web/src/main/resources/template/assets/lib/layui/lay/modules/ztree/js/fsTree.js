layui.define(['layer', "fsCommon"], function(exports) {
	var layer = layui.layer, fsCommon = layui.fsCommon, FsTree = function() {
		this.config = {
			...
			onDrop : undefined,// 用于捕获节点拖拽操作结束的事件回调函数
			...
			onRightClick : undefined,// 右键
			expandSpeed : ''// zTree 节点展开、折叠时的动画速度，设置方法同 JQuery 动画效果中 speed
							// 参数。三种预定速度之一的字符串("slow", "normal", or "fast")
		}
	};
			...
			callback : {
				...
				onAsyncSuccess : function() {
					...
					if ("checkbox" == checkType || "radio" == checkType) {
						var zTree = $.fn.zTree.getZTreeObj(_this.config.id);
						var node = zTree.getNodeByParam(_this.config.treeIdKey,
								0, null);
						if (null != node) {
							var checkedNode = zTree.getCheckedNodes(true);
							if (checkedNode.length > 1
									|| (checkedNode.length == 1 && checkedNode[0][_this.config.treeIdKey] !== 0)) {
								node.checked = true;
							} else {
								node.checked = false;
							}
	...
	FsTree.prototype.refresh = function(tid, callback) {
		var _this = this;
		var zTree = $.fn.zTree.getZTreeObj(_this.config.id), type = "refresh", silent = false;
		...
		if (!isNull(tid) && !isNull(treeId)) {
			// 选中刷新的菜单
			var node3 = zTree.getNodeByParam(_this.config.treeIdKey, treeId,
					null);
