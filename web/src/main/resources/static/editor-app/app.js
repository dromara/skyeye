            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    var offset = jQuery("#editor-header").offset();//操作栏
	                    var propSectionWidth = jQuery('#propertySection').width();//属性栏
	                    var canvas = jQuery('#canvasSection');//画布

	                    if (offset == undefined || offset === null
	                        || propSectionWidth === undefined || propSectionWidth === null
	                	...
	                	//计算画布宽度并重置
	                    var totalAvailable = jQuery(window).width() - 160;
	                    canvas.width(totalAvailable - propSectionWidth);
