...
layui.config({
	...
	table.render({
	    ...
	    elem: '#messageTable',
	    method: 'post',
	    url: reqBasePath + 'crmcustomerindustry001',
	...
	function del(data, obj){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "crmcustomerindustry005", params: {rowId: data.id}, type: 'json', callback: function(json){
	...
	function up(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '所属行业上线'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "crmcustomerindustry006", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
	...
	function down(data, obj){
		...
		layer.confirm(msg, {icon: 3, title: '所属行业下线'}, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "crmcustomerindustry007", params: {rowId: data.id}, type: 'json', callback: function(json){
    			if(json.returnCode == 0){
