	...
	layui.use(['form'], function (form) {
		...
		$("body").on("click", "#conmunicate", function(){
			...
			layer.confirm(msg, { icon: 3, title: '初期沟通' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity019", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
		...
		$("body").on("click", "#quotedPrice", function(){
			...
			layer.confirm(msg, { icon: 3, title: '方案与报价' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity020", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
		...
		$("body").on("click", "#tender", function(){
			...
			layer.confirm(msg, { icon: 3, title: '竞争与投标' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity021", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
		...
		$("body").on("click", "#negotiate", function(){
			...
			layer.confirm(msg, { icon: 3, title: '商务谈判' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity022", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
		...
		$("body").on("click", "#turnover", function(){
			...
			layer.confirm(msg, { icon: 3, title: '成交' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity023", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
		...
		$("body").on("click", "#losingTable", function(){
			...
			layer.confirm(msg, { icon: 3, title: '丢单' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity024", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
		...
		$("body").on("click", "#layAside", function(){
			...
			layer.confirm(msg, { icon: 3, title: '搁置' }, function (i) {
				layer.close(i);
	            AjaxPostUtil.request({url:reqBasePath + "opportunity025", params:{rowId: stateChangeData.id}, type: 'json', callback: function(json){
	    			if(json.returnCode == 0){
	    				parent.layer.close(index);
		 	        	parent.refreshCode = '0';
	    			}else{
