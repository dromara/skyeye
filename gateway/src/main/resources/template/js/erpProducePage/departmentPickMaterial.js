...
layui.config({
	...
	function initCustomDocumentaryByType(year){
		AjaxPostUtil.request({url:reqBasePath + "erpproduce001", params: {year: year}, type: 'json', callback: function(json){
	...
	function renderCharts1(rows) {
		...
		$.each(rows, function(i, item){
			if(i == 0){
				...
				$.each(item.yearPickMaterialNum, function(j, bean){
		...
		var option = getOption('当前部门领料分析', '统计在指定年份当前部门领料的数量分析', name, materialName, num, 'line');
		$.each(rows, function(i, item){
			if(i != 0){
				materialName = item.materialName;
				materialNameList.push(materialName);
    ...
    exports('departmentPickMaterial', {});
});
