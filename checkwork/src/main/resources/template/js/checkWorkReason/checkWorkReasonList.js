...
layui.config({
	...
	table.render({
	    ...
	    elem: '#messageTable',
	    method: 'post',
	    url: reqBasePath + 'checkworkreason001',
	...
	function delet(data){
		layer.confirm(systemLanguage["com.skyeye.deleteOperationMsg"][languageType], {icon: 3, title: systemLanguage["com.skyeye.deleteOperation"][languageType]}, function(index){
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "checkworkreason003", params: {rowId: data.id}, type: 'json', method: "DELETE", callback: function(json){
	...
	function up(data){
		...
		layer.confirm(msg, { icon: 3, title: '上线操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "checkworkreason004", params: {rowId: data.id}, type: 'json', method: "PUT", callback: function(json){
    			if(json.returnCode == 0){
	...
	function down(data){
		...
		layer.confirm(msg, { icon: 3, title: '下线操作' }, function (index) {
			layer.close(index);
            AjaxPostUtil.request({url: reqBasePath + "checkworkreason005", params: {rowId: data.id}, type: 'json', method: "PUT", callback: function(json){
    			if(json.returnCode == 0){
	...
	function upMove(data){
        AjaxPostUtil.request({url: reqBasePath + "checkworkreason008", params: {rowId: data.id}, type: 'json', method: "PUT", callback: function(json){
			if(json.returnCode == 0){
	...
	function downMove(data){
        AjaxPostUtil.request({url: reqBasePath + "checkworkreason009", params: {rowId: data.id}, type: 'json', method: "PUT", callback: function(json){
			if(json.returnCode == 0){
