...
layui.config({
    ...
    AjaxPostUtil.request({url: reqBasePath + "login002", params: {}, type: 'json', callback: function(json) {
        if(json.returnCode == 0) {
            $("#useTitle").html("用户销假申请单-" + getYMDFormatDate() + '-' + json.bean.userName);
    ...
    function saveData(subType, approvalId){
        ...
        AjaxPostUtil.request({url: reqBasePath + "checkworkcancelleave002", params: params, type: 'json', method: 'POST',  callback: function(json) {
            if(json.returnCode == 0) {
